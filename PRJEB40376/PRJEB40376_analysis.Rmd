...
for(i in 1:length(seu.pool.list)){
  ptprc.pool.minus <- readRDS(paste0('Ptprc/Ptprc_', pool.names[i], '_100minus_raw_matrix.rds'))
  ptprc.pool.plus <- readRDS(paste0('Ptprc/Ptprc_', pool.names[i], '_100plus_raw_matrix.rds'))
...
subset.stats <- data.frame(percentages = seq(25,100,25),
                           read.count.div.cell = read.counts$read.count,
...
```{r, warning = F}
CD45RA.ppc.nls.reg <- nls(CD45RA.perc.pos.cells ~ a*read.count.div.cell/(b + read.count.div.cell), data = subset.stats, start = list(a=0.01, b=10000000))
CD45RO.ppc.nls.reg <- nls(CD45RO.perc.pos.cells ~ a*read.count.div.cell/(b + read.count.div.cell), data = subset.stats, start = list(a=0.01, b=10000000))
CD45RA.avg.nls.reg <- nls(CD45RA.avg ~ a*read.count.div.cell/(b + read.count.div.cell), data = subset.stats, start = list(a=0.01, b=10000000))
CD45RO.avg.nls.reg <- nls(CD45RO.avg ~ a*read.count.div.cell/(b + read.count.div.cell), data = subset.stats, start = list(a=0.01, b=10000000))
...
```{r, warning=F}
new.interval <- expand.grid(read.count.div.cell = seq(0, 100, 1)*10^8)
