...
DimPlot(HIV.integ.data, group.by = 'orig.ident', label = T) + ggtitle('HIV-CD4+ analysis - unstimulated healthy controls\n - integration')
```
...
We now try to fit model Y = a*X/(X+b). This model was chosen because all values here should exhibit asymptotic behavior and this model has shown behavior than regular asymptotic regression model. We use nls package for this purpose. First show data to plot again.
