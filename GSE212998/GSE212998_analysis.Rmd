...
```{r}
ptprc_w1_pre_inf <- readRDS('../Data_sets/Human5/GSE212998/Ptprc_GFP-pos-w1.rds')
GFP_pos_w1_pre_inf[['Ptprc']] <- CreateAssayObject(ptprc_w1_pre_inf)

ptprc_w2_pre_inf <- readRDS('../Data_sets/Human5/GSE212998/Ptprc_GFP-pos-w2.rds')
GFP_pos_w2_pre_inf[['Ptprc']] <- CreateAssayObject(ptprc_w2_pre_inf)

ptprc_w1_inf <- readRDS('../Data_sets/Human5/GSE212998/Ptprc_GFP-pos-w1-expand.rds')
GFP_pos_w1_inf[['Ptprc']] <- CreateAssayObject(ptprc_w1_inf)

ptprc_w2_inf <- readRDS('../Data_sets/Human5/GSE212998/Ptprc_GFP-pos-w2-expand.rds')
GFP_pos_w2_inf[['Ptprc']] <- CreateAssayObject(ptprc_w2_inf)

ptprc_inf_neg <- readRDS('../Data_sets/Human5/GSE212998/Ptprc_GFP-neg-expand.rds')
GFP_neg_inf[['Ptprc']] <- CreateAssayObject(ptprc_inf_neg)
```
...
```{r}
for(i in 1:length(enter.data.list)){
  CD45er.probs <- read.csv(paste0('Ptprc_CD45er/', names(enter.data.list)[i] , '_iso_probs.csv'))
...
```{r}
saveRDS(enter.integ.data, 'Data_sets/GSE212998_integrated_data.rds')
```
...
```{r}
enter.integ.data.counts <- GetAssayData(enter.integ.data, assay = 'RNA', slot = 'data')
```
...
```{r}
enter.integ.data.Immgen <- SingleR(enter.integ.data.counts, MonacoData, MonacoData$label.main, fine.tune = T)
enter.integ.data$annotation.ImmGen <- enter.integ.data.Immgen$pruned.labels

enter.integ.data.Immgen.fine <- SingleR(enter.integ.data.counts, MonacoData, MonacoData$label.fine, fine.tune = T)
enter.integ.data$annotation.ImmGen.fine <- enter.integ.data.Immgen.fine$pruned.labels
```

We now show generated annotations on UMAP plot.

```{r, fig.width = 10}
DimPlot(enter.integ.data, group.by = 'annotation.ImmGen') + ggtitle('Annotation Immgen')
DimPlot(enter.integ.data, group.by = 'annotation.ImmGen.fine') + ggtitle('Annotation Immgen (fine)')
```

The annotation is incorrect (there are no CD4 T cells), but with its help we can create manual annotation.

```{r}
enter.integ.data$annotation.manual <- 'Effector Memory CD8 T cells'
enter.integ.data$annotation.manual[enter.integ.data$seurat_clusters %in% c(4,6)] <- 'Naive CD8 T cells'
enter.integ.data$annotation.manual[enter.integ.data$seurat_clusters %in% c(5)] <- 'MAIT'
enter.integ.data$annotation.manual[enter.integ.data$seurat_clusters %in% c(1,3,9,10,11,12)] <- 'Effector Memory/Teminal Effector CD8 T cells'
enter.integ.data$annotation.manual[enter.integ.data$seurat_clusters %in% c(13)] <- 'Proliferating CD8T cells'
enter.integ.data$annotation.manual[enter.integ.data$seurat_clusters %in% c(14)] <- 'FOXP3+ CD8+ T cells'
```

Now show what we made.

```{r}
DimPlot(enter.integ.data, group.by = 'annotation.manual')
```
...
features.to.plot = c('SELL', 'CCL5', 'CCR7', 'GZMA', 'ISG15', 'IFNG', 'TNF', 'CXCR4', 'IL7R', 'GZMA', 'TCF7', 'IFIT3', 'IFIT2', 'KLF2')
breaks <- c(4, 6, 4, 6, 4, 8, 6, 3, 4, 6, 3, 4, 4, 3, 4, 4)
