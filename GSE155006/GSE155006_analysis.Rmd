...
for(i in 1:length(mm.mog.list)){
  mm.mog.ptprc <- readRDS(paste0('Ptprc/Ptprc_', mog.names[i], '.rds'))
...
```{r}
mm.mog.integ.data.data <- GetAssayData(mm.mog.integ.data, assay = 'RNA', slot = 'data')
```
...
```{r}
mm.mog.integ.data.Immgen <- SingleR(mm.mog.integ.data.data, ImmGenData, ImmGenData$label.main, fine.tune = T)
mm.mog.integ.data$annotation.ImmGen <- mm.mog.integ.data.Immgen$pruned.labels

mm.mog.integ.data.Immgen.fine <- SingleR(mm.mog.integ.data.data, ImmGenData, ImmGenData$label.fine, fine.tune = F)
mm.mog.integ.data$annotation.ImmGen.fine <- mm.mog.integ.data.Immgen.fine$labels
```

From the second annotation we remove all cell types where count is lower than 100.

```{r}
mm.mog.integ.data$annotation.ImmGen.filt <- mm.mog.integ.data$annotation.ImmGen.fine
mm.mog.integ.data$annotation.ImmGen.filt[mm.mog.integ.data$annotation.ImmGen.filt %in% names(table(mm.mog.integ.data$annotation.ImmGen.filt)[table(mm.mog.integ.data$annotation.ImmGen.filt) < 200])] <- 'Other'
```

We now show generated annotations on UMAP plot.

```{r, fig.width = 12}
DimPlot(mm.mog.integ.data, group.by = 'annotation.ImmGen', label = T) + ggtitle('Annotation Immgen')
DimPlot(mm.mog.integ.data, group.by = 'annotation.ImmGen.filt', label = T) + ggtitle('Annotation Immgen (filt)')
```

Update data set with annotation.
...
for(i in 1:length(feature.to.list)){
  fb.plot <- FeaturePlot(mm.mog.integ.data, feature.to.list[i], min.cutoff = 0) + 
  ...
  fb.plot.blank <- FeaturePlot(mm.mog.integ.data, feature.to.list[i], min.cutoff = 0) + 
    ggtitle('') + xlab('') + ylab('') +
    scale_colour_gradient(breaks = c(0,lims[i]), low = 'grey85', high = colors[i]) +
    scale_x_continuous(breaks=c(-15, 0, 10)) +
    scale_y_continuous(breaks=c(-15, 0, 5)) +
