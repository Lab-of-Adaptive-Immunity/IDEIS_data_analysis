...
```{r}
fb.plot.gzma <- FeaturePlot(HIV.integ.data, 'IL7R', min.cutoff = 0) + 
...
```{r}
fb.plot.gzma.blank <- FeaturePlot(HIV.integ.data, 'IL7R', min.cutoff = 0) + 
  ggtitle('') + xlab('') + ylab('') +
  scale_colour_gradient(breaks = c(0,4), low = 'grey85', high = 'blue') +
  theme(plot.title = element_text(hjust = 0.5),
        axis.text.x=element_blank(),
...
```{r}
tiff('GSE187515_scatter_RO_blank.tiff', width = 6, height = 6, units = 'in', res = 200)
print(scatterplot.lin.RO.blank)
dev.off()
```
